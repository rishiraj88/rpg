        ...
        while (true) {
            try { //WASD keys for gaming
                ...
                switch (c) {
                    ...
                    case 'X', 'x' -> FlyweightProvider.getSavegameProvider().saveGame(player);
                    case '\n' -> {}
                    case 'q' -> System.exit(0);
                    default ->
                            System.out.println("invalid command. Please press W, A, S, D or F to play. Press X to save game.");
                }
            } catch (IOException e) {
